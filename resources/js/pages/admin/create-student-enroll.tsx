...
export default function CreateStudentEnroll() {
    ...
    function getAddressFieldValue(address: CommonAddress, field: keyof Pick<CommonAddress, 'address_zone_street' | 'address_barangay' | 'address_municipality' | 'address_province'>) {
        return (address[field] || '').trim().toLowerCase();
    }

    function findCommonAddress(field: keyof Pick<CommonAddress, 'address_zone_street' | 'address_barangay' | 'address_municipality' | 'address_province'>, value: string) {
        ...
        return commonAddresses.find((address) => getAddressFieldValue(address, field).startsWith(query)) ?? null;
    }
    ...
    function submit(e: React.FormEvent) {
        ...
        if (!hasOnlyDigits(form.contact_number)) {
            window.dispatchEvent(new CustomEvent('local-toast', { detail: { message: 'Contact number can only contain digits.', type: 'error' } }));

            return;
        }

        if (!hasOnlyDigitsAndHyphen(form.last_school_year)) {
            window.dispatchEvent(new CustomEvent('local-toast', { detail: { message: 'Last school year can only contain digits and hyphens.', type: 'error' } }));

            return;
        }

        if (!hasOnlyDigits(form.last_grade_level)) {
            window.dispatchEvent(new CustomEvent('local-toast', { detail: { message: 'Last grade level can only contain digits.', type: 'error' } }));
    ...
    return (
        <>
            ...
            <PortalPageShell title="Create & Enroll Student" description="Create a new student account and enroll them into a selected class section." showBackLink={false}>
                <form onSubmit={submit} className="rounded-2xl border border-sidebar-border/70 bg-white p-5 shadow-sm dark:border-sidebar-border dark:bg-sidebar">
                    <div className="grid gap-4 md:grid-cols-2">
                        ...
                        <label className="space-y-2 md:col-span-2">
                            <span className={labelClass}>Name Breakdown <span className="text-destructive">*</span></span>
                        ...
                        <label className="space-y-2 text-sm">
                            ...
                            <input
                                ...
                                onChange={(e) => setForm({ ...form, contact_number: keepDigits(e.target.value) })}
                            ...
                            <div className="grid gap-4 md:grid-cols-2">
                                <label className="space-y-2 text-sm md:col-span-2">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => setForm({ ...form, address_zone_street: e.target.value })}
                                        onBlur={(e) => handleAddressBlur('address_zone_street', e.target.value)}
                                        list="common-address-zone-street"
                                        className={inputClass}
                                    />
                                    <datalist id="common-address-zone-street">
                                        {commonAddresses
                                            .map((address) => address.address_zone_street)
                                ...
                                <label className="space-y-2 text-sm">
                                    <span className={labelClass}>Barangay <span className="text-destructive">*</span></span>
                                    <input
                                        value={form.address_barangay}
                                        onChange={(e) => setForm({ ...form, address_barangay: e.target.value })}
                                        onBlur={(e) => handleAddressBlur('address_barangay', e.target.value)}
                                        list="common-address-barangay"
                                        className={inputClass}
                                    />
                                    <datalist id="common-address-barangay">
                                        {commonAddresses
                                            .map((address) => address.address_barangay)
                                            .filter((value, index, values) => Boolean(value) && values.indexOf(value) === index)
                                            .map((value) => (
                                                <option key={value} value={value as string} />
                                            ))}
                                ...
                                <label className="space-y-2 text-sm">
                                    <span className={labelClass}>Municipality <span className="text-destructive">*</span></span>
                                    <input
                                        value={form.address_municipality}
                                        onChange={(e) => setForm({ ...form, address_municipality: e.target.value })}
                                        onBlur={(e) => handleAddressBlur('address_municipality', e.target.value)}
                                        list="common-address-municipality"
                                        className={inputClass}
                                    />
                                    <datalist id="common-address-municipality">
                                        {commonAddresses
                                            .map((address) => address.address_municipality)
                                            .filter((value, index, values) => Boolean(value) && values.indexOf(value) === index)
                                            .map((value) => (
                                                <option key={value} value={value as string} />
                                            ))}
                                ...
                                <label className="space-y-2 text-sm md:col-span-2">
                                    <span className={labelClass}>Province <span className="text-destructive">*</span></span>
                                    <input
                                        value={form.address_province}
                                        onChange={(e) => setForm({ ...form, address_province: e.target.value })}
                                        onBlur={(e) => handleAddressBlur('address_province', e.target.value)}
                                        list="common-address-province"
                                        className={inputClass}
                                    />
                                    <datalist id="common-address-province">
                                        {commonAddresses
                                            .map((address) => address.address_province)
                                            .filter((value, index, values) => Boolean(value) && values.indexOf(value) === index)
                                            .map((value) => (
                                                <option key={value} value={value as string} />
                                            ))}
                        ...
                        <div className="md:col-span-2 rounded-2xl border border-sidebar-border/60 bg-sidebar/30 p-4">
                            ...
                            <div className="grid gap-4 md:grid-cols-2">
                                <label className="space-y-2 text-sm md:col-span-2">
                                    <span className={labelClass}>Previous School <span className="text-destructive">*</span></span>
                                    <input value={form.previous_school} onChange={(e) => setForm({ ...form, previous_school: e.target.value })} className={inputClass} />
                                </label>

                                <label className="space-y-2 text-sm">
                                    <span className={labelClass}>Last School Year <span className="text-destructive">*</span></span>
                                    <input
                                        inputMode="numeric"
                                        pattern="[0-9-]*"
                                        value={form.last_school_year}
                                        onChange={(e) => setForm({ ...form, last_school_year: keepDigitsAndHyphen(e.target.value) })}
                                        className={inputClass}
                                    />
                                </label>

                                <label className="space-y-2 text-sm">
                                    <span className={labelClass}>Last Year Level <span className="text-destructive">*</span></span>
                                    <input
                                        inputMode="numeric"
                                        pattern="[0-9]*"
                                        value={form.last_grade_level}
                                        onChange={(e) => setForm({ ...form, last_grade_level: keepDigits(e.target.value) })}
                                        className={inputClass}
                                    />
                                </label>
                        ...
                        <label className="space-y-2 text-sm">
                            <span className={labelClass}>Email <span className="text-destructive">*</span></span>
                            <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className={inputClass} />
                        </label>

                        <label className="space-y-2 text-sm">
                            <span className={labelClass}>Student ID Preview</span>
                            <input readOnly value={form.student_id} className={`${inputClass} bg-muted/60 text-foreground shadow-sm`} />
                        </label>

                        {/* Passwords last */}
                        <label className="space-y-2 text-sm">
                            <span className={labelClass}>Password <span className="text-destructive">*</span></span>
                        ...
                        <label className="space-y-2 text-sm">
                            <span className={labelClass}>Confirm Password <span className="text-destructive">*</span></span>
                            <div className="relative">
                        ...
                        <div className="md:col-span-2 flex justify-end pt-2">
                            <button
                                disabled={submitting || !composedName || !form.email || !form.password || !form.password_confirmation || !form.address_barangay || !form.address_municipality || !form.address_province || !form.previous_school || !form.last_school_year || !form.last_grade_level}
                            ...
                            <div className="flex h-40 items-center justify-center overflow-hidden rounded-2xl border border-border/70 bg-background shadow-sm">
                                {avatarPreview ? (
                                    ...
                                    <div className="px-4 text-center">
                                        <div className="text-sm font-medium text-foreground">No photo selected</div>
                                        <div className="mt-1 text-xs text-muted-foreground">Drop or choose a student photo</div>
                                    </div>
                                )}
                            </div>
                            <div className="flex flex-col justify-between gap-3">
                                <div>
                                    <p className="text-sm font-medium text-foreground">Upload a profile image</p>
                                    <p className="mt-1 text-xs leading-5 text-muted-foreground">PNG, JPG, JPEG, or WEBP. Drag and drop or select a file.</p>
                                </div>
                                <input id="avatar" type="file" accept="image/*" onChange={(e) => handleAvatarFile(e.target.files?.[0] ?? null)} className="block w-full text-sm text-muted-foreground file:mr-4 file:rounded-full file:border-0 file:bg-foreground file:px-4 file:py-2 file:text-sm file:font-medium file:text-background hover:file:opacity-90" />
                            </div>
                        </div>
                    </div>
                </form>
